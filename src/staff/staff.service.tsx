...
import {
  TeachersHeaders,
  translations,
} from "src/common/translations/translationObjects";
...
export class StaffService extends UserService {
  constructor(
    ...
    @Inject(UserSchoolService)
    protected readonly userSchoolService: UserSchoolService,
    ...
    @Inject(StarredUserClassesService)
    protected readonly starredUserClassesService: StarredUserClassesService,
  ...
  async registerUser(
    roleId: number,
    user: { username: string; password: string; schoolId: number }
  ) {
    try {
      ...
      this.userSchoolService.connectUserToSchool(
        roleId,
        user.schoolId,
        userData.id
      );
  ...
  async isFirstLogin(userId: string, isAdmin: boolean) {
    ...
    const user = await this.staffRepository
      .findOneBy({ id: userId })
      .catch((e) => {
        throw e;
      });
  ...
  async adminGetTeachers(adminId: string, schoolId: number) {
    ...
    return this.staffRepository
      ...
      .where(
        new Brackets((qb) => {
          qb.where(`user_role.id=${RoleIds.ADMIN}`).orWhere(
            `user_role.id=${RoleIds.TEACHER}`
          );
        })
      )
      .andWhere("user_school.school_id=:schoolId", { schoolId })
  ...
  async updateTeacherDetails(
    ...
    const teacher = await this.staffRepository.findOneBy({
      username: teacherDetailsDto.username,
    });
  ...
  async updateTeacherDetailsByAdmin(
    teacherId: string,
    saveTeacherDto: SaveTeacherDto,
    schoolId: number
  ) {
    const teacherExist = await this.staffRepository.findOneBy({
      id: teacherId,
    });
    ...
    await this.classesService.updateTeacherClasses(
      schoolId,
      teacherId,
      saveTeacherDto.classes
    );
  }
  ...
  async deleteTeacher(teacherId: string, adminsSchoolId: number) {
    ...
    const teachersSchoolIds = (
      await this.userSchoolService.findUserSchoolIds(teacherId)
    ).map((val) => val.schoolId);
    if (!teachersSchoolIds)
      throw new BadRequestException("no user school relation found");

    if (!teachersSchoolIds.includes(adminsSchoolId))
      throw new UnauthorizedException(
        "admin is not in the same school as the teacher"
      );
    ...
    if (teachersSchoolIds.length === 1) {
      ...
      this.accessTokenService.removeAccessTokenByUserId(
        teacherId,
        adminsSchoolId
      );
      ...
      return await this.userSchoolService.removeUserSchoolRelation(
        teacherId,
        adminsSchoolId
      );
    }
  ...
  async softDeleteTeachers(
    ...
    const teacherIdsWithMatchingSchool =
      await this.userSchoolService.findTeacherIdsWithMatchingSchool(
        teacherIds,
        adminsSchoolId
      );
    ...
    for (const teacherId of teacherIds) {
      try {
        ...
        this.logger.error(
          `Error deleting teacher with ID ${teacherId}: ${error.message}`
        );
      }
    }
  }
  ...
  async deleteTeacherSoft(
    ...
    const teachersSchoolIds =
      (await this.userSchoolService.findUserSchoolIds(teacherId)).map(
        (val) => val.schoolId
      ) || [];
    ...
    const classIds = teachersClassIds.flatMap((teacher) =>
      teacher.classes.map((classObj) => classObj.id)
    );
  ...
  validateAdminSchool(
    teacherId: string,
    teacherIdsWithMatchingSchool: string[]
  ): void {
    if (!teacherIdsWithMatchingSchool.includes(teacherId)) {
      throw new UnauthorizedException(
        "Admin is not in the same school as teacher ID: " + teacherId
      );
    }
  }
  ...
  async performSoftDelete(
    ...
    teacher.classes = teacher.classes.filter(
      (cls) => cls.schoolId !== adminsSchoolId
    );
    teacher.studyGroups = teacher.studyGroups.filter(
      (cls) => cls.schoolId !== adminsSchoolId
    );

    await this.starredUserClassesService.removeTeacherClassesRelations(
      teacherId,
      teachersClassIds
    );

    await Promise.all([
      this.userSchoolService.removeUserSchoolRelation(
        teacherId,
        adminsSchoolId
      ),
      this.accessTokenService.removeAccessTokenByUserId(
  ...
  async deleteTeachersExceptById(
    body: AdminTableDto<TeacherRow>,
    schoolId: number
  ) {
    try {
      ...
      if (userSearch) {
        queryBuilder.andWhere(
          "CONCAT(staff.firstName, ' ', staff.lastName) LIKE :userSearch",
          {
      ...
      if (filters.length) {
        ...
        filters.forEach((element) => {
          queryBuilder.andWhere("classObj.grade IN (:...grade)", {
            grade: element.optionKey,
          });
        });
      }
      ...
      console.log(
        `Found ${teachersToBeDeleted.length} teacher entities to be deleted`
      );
  ...
  async addOrRemoveClassFromUser(
    classId: number,
    userId: string,
    schoolId: number,
    action: StarredActions
  ) {
    const userRelated = await this.userSchoolService.checkIfUserRelatedToSchool(
      schoolId,
      userId
    );
    if (!userRelated)
      throw new UnauthorizedException("user not related to school");
    const classRelated = await this.classesService.classRelatedToSchool(
      schoolId,
      classId
    );
    if (!classRelated)
      throw new UnauthorizedException("class not related to school");

    await this.starredUserClassesService.addOrRemoveClassForUser(
      classId,
      userId,
      action
    );
  ...
  // needs fixing
  async addOrRemoveStudyGroupFromUser(
    studyGroup: number,
    userId: string,
    schoolId: number,
    action: StarredActions
    ...
    if (!userRelated)
      throw new UnauthorizedException("user not related to school");
    const classRelated = await this.studyGroupService.groupRelatedToSchool(
      schoolId,
      studyGroup
    );
    if (!classRelated)
      throw new UnauthorizedException("study group not related to school");
    const queryBuilder = this.staffRepository
      .createQueryBuilder("staff")
  ...
  async passwordReset(email: string, lang: Language) {
    ...
    if (user) {
      const token = await (
        await this.generateVerificationTokenAndSave({ username: email })
      )[VERIFICATION_TOKEN];
      ...
      this.mailService.send({
        ...
        html: this.mailService.createHtmlMessage(
          text,
          translation.password_reset.password_reset,
          email,
          lang
        ),
      });
   ...
   */ async uploadTeachersEXCEL(
    sheet: ExcelPipeResult<TeachersHeaders>["sheet"],
    schoolId: number
  ) {
    try {
      let gptTokens = 0;
      ...
      for (const row of sheet) {
        ...
        const { grade, classIndex } = this.processGradeAndClass(
          rawClassIndex,
          rawGrade
        );
      ...
      this.logger.log(
        `~~~ Inserting ${teachersToInsert.length} teachers to school ${schoolId}`
      );

      const foundOrCreatedClasses = await this.findOrCreateClasses(
        classesSet,
        schoolId
      );

      const foundTeachers = await this.staffRepository.find({
        where: teachersSearch,
      });
      ...
      this.logger.log(
        `Created ${newTeachersCount} teachers, updated ${updatedTeachersCount}`
      );

      return {
        updated: updatedTeachersCount,
        newRecords: newTeachersCount,
        buffer,
      };
    } catch (err) {
  ...
  async findOrCreateClasses(
    ...
    let foundOrCreatedClasses:
      | Record<`${SchoolGrades}-${number}`, Classes["id"]>
      | undefined[] = [];
   ...
   */
  processGradeAndClass(
    rawClassIndex: string | undefined,
    ...
    if (rawClassIndex || rawGrade) {
      try {
        ...
        classIndex =
          Number(rawGrade?.match(/\d+/)?.join("")) || Number(rawClassIndex);
        if (!classIndex) {
        ...
        const errorMessage =
          err instanceof BadRequestException
            ...
            : "An error occurred";
        this.logger.error(
          "Error handling teacher class and grade: " +
        ...
        if (
          ...
          teacher.classes = [
            ...classes,
            {
              ...
              classIndex: teacher.classes[0].classIndex,
            },
          ];
        }
      } else {
  ...
  async sendTeacherPassEmail(
    ...
    try {
      ...
      this.logger.error(
        "Error sending email to teachers: " + JSON.stringify(error)
      );
    }
  }
    ...
    try {
      ...
      if (user) {
        ...
        const isAdminAlready = user.schools?.find(
          (school) =>
            school.schoolId === schoolId && school.roleId == RoleIds.ADMIN
        );
        if (isAdminAlready) {
          //throw 409 error
        ...
        if (
          ...
          user.schools.some(
            (school) =>
              school.schoolId == schoolId && school.roleId == RoleIds.TEACHER
          )
        ) {
        ...
        await this.userSchoolService.connectUserToSchool(
          roleId,
          schoolId,
          newUser.id
        );
  ...
  formatExcelTeacherData(data: DeepPartial<Staff>[], lang: Language) {
    ...
    data.forEach((val) => {
      formattedData.push({
        [translation.teachersHeaders.fullName]:
      ...
      select: {
        ...
        studentsGoodPoints: {
          student: {
            firstName: true,
            lastName: true,
            class: { grade: true, classIndex: true },
          },
          id: true,
          gpText: true,
          created: true,
  ...
  async createTeacherReportXlsx(
    ...
    const teacherGPData = await this.getGPsByTeacher(
      schoolId,
      teacherId,
      dates
    );
    ...
    try {
      ...
      for (const school of schools) {
        ...
        for (const teacher of schoolTeachers) {
          const students =
            await this.studentService.getStudentsWithoutMonthlyGpsByClassIds(
          ...
          const text = (
            <Container>
              <Text className="text-black text-[18px] leading-[30px]">
                {mails.for_your_info}
              </Text>
              <Text className="text-black text-[18px] leading-[24px] font-bold">{`${grades[currClass.grade]}-${currClass.classIndex}`}</Text>
          ...
          this.logger.log(
            `Sending monthly report to ${teacher.username} for ${students.length} students`
          );
  ...
  async loginByIdm(idmUser: IdmUser, res: Response) {
    ...
    if (userInst) {
      ...
      const schoolIds = await this.schoolService.findSchoolsByCodes(
        idmUser.schoolCodes
      );
      if (!schoolIds.length)
        throw new UnauthorizedException("school does not exist in our system");
