...
export class AdminActionsService {
  ...
  async updateGradeUpDate(manager: EntityManager, adminId: string) {
    return manager.getRepository(AdminActions).save({
      actionType: SuperAdminActions.NEW_YEAR,
      operatingAdminId: adminId,
    });
  }
  ...
  async canRunNewYear() {
    ...
    const daysDiff =
      (Date.now() - instance.created.getTime()) / (1000 * 3600 * 24);
