...
export class ClassesService {
  ...
  async getSchoolClassId(
    grade: SchoolGrades,
    classIndex: number,
    schoolId: number
  ) {
    return await this.classesRepository.findOne({
      where: { grade, classIndex, schoolId },
      select: ["id"],
    });
  }

  async getSchoolClassIdsByGrades(
    ArrayGrade: SchoolGrades[],
    schoolId: number
  ): Promise<Classes["id"][]> {
  ...
  async adminFetchClasses(
    ...
    if (fetchedClasses.length)
      classes = classes.andWhere(
        `class.id NOT IN (${fetchedClasses.join(",")})`
      );
      ...
      classes = classes
        .andWhere(
          new Brackets((qb) =>
            qb
              .where(
                'CONCAT(Teacher.first_name," ",Teacher.last_name) REGEXP :tName'
              )
    ...
    if (grade && grade !== "null")
      classes = classes.andWhere("grade=:grade", { grade });
    if (cIndex && !isNaN(cIndex))
      classes = classes.andWhere("class_index=:cIndex", { cIndex });
    classes = classes
      .orderBy("class.grade")
  ...
  async createClassesReportXlsx(
    schoolId: number,
    classId: number,
    dates: DateForReportDto,
    lang: Language
  ) {
    return this.StudentsService.createStudentsReportXlsx(
      schoolId,
      classId,
      [],
      dates,
  ...
  async allStudentGradeUp(manager: EntityManager) {
    ...
    try {
      await manager.query(
        "ALTER TABLE classes DROP INDEX IDX_9a1024736a466e0361b226cef9"
      );
      ...
      if (
        ...
        err.sql.includes("`grade` IS NOT NULL")
      ) {
  ...
  async adminAddClasses(
    classes: ExcelSheet<ClassesHeaders>,
    headerLang: Language,
    schoolId: number
  ) {
    const newClasses = await Promise.all(
      classes.map(async (classroom) => {
        ...
        const teacher = await this.staffService.findTeacherWithSchool(
          { lastName, firstName },
          schoolId
        );
  ...
  async getTeacherStarredClasses(teacherId: string, schoolId: number) {
  ...
  async findOrCreateByStrings(
    ...
    const classes = await this.classesRepository
      ...
      .andWhere("CONCAT(grade, '-',class_index) in (:...classesArr)", {
        classesArr,
      })
      .getMany();
  ...
  async findOrCreateClassByGradeAndIndex(
    ...
    try {
      ...
      this.logger.error(
        `theres has been an error upserting  the class: ${error}`
      );
      return error;
    }
  }
      ...
      for (let i = 0; i < classes.length; i++) {
        ...
        query.orWhere(
          new Brackets((qb) => {
            ...
            qb.andWhere(`classes.grade = :grade${i}`, {
              [`grade${i}`]: c.grade,
            });
            qb.andWhere(`classes.class_index = :classIndex${i}`, {
              [`classIndex${i}`]: c.classIndex,
            });
          })
        );
      }
  ...
  async getSchoolGradeRange(
    schoolId: number
  ): Promise<{ smallestGrade: SchoolGrades; largestGrade: SchoolGrades }> {
    ...
    return {
      smallestGrade: grade.smallestGrade,
      largestGrade: grade.largestGrade,
    };
  }
  ...
  async deleteAdminClasses(
    body: AdminTableDto<ClassesTableRow>,
    schoolId: number
  ) {
    ...
    if (body.params.allChecked) {
      ...
      const classesToDelete = await this.classesRepository
        ...
        .andWhere(
          `CONCAT(teacher.firstName, " ", teacher.lastName) LIKE :name`,
          { name: `%${userSearch}%` }
        )
        .getMany()
      ...
      if (userSearch) {
        deleteClassesQuery = deleteClassesQuery.andWhere(
          "id IN (:...classesToDelete)",
          { classesToDelete }
        );
      }
      ...
      deleteClassesQuery = deleteClassesQuery.andWhere("id IN (:...selected)", {
        selected,
      });
      deleteClassesQuery.execute();
    }
  ...
  async uploadClassesEXCEL(
    sheet: ExcelSheet<ClassesHeaders>,
    schoolId: number
  ) {
    let gptTokens = 0;
    ...
    try {
      for (let i = 0; i < sheet.length; i++) {
        ...
        const foundClass = await this.classesRepository.findOneBy({
          grade,
          classIndex,
          schoolId,
        });
      ...
      classesToInsert.map((classObj) =>
        classObj.id ? updatedClassesCount++ : ""
      );
      ...
      this.logger.log(
        `Created ${newClassesCount} classes, updates ${updatedClassesCount} `
      );

      return { updated: updatedClassesCount, newRecords: newClassesCount };
    } catch (error) {
      this.logger.error(
        `Error uploading classes to school ${schoolId} from Excel sheet: ${error} `
      );
      throw error;
    }
  }
