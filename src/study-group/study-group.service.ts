...
export class StudyGroupService {
  ...
  async addOrEditStudyGroup(
    body: AdminAddOrEditStudyGroupDto,
    schoolId: number
  ) {
    if (body.id) {
      ...
      const newGradesArray = body.studyGroupGrades.map(
        (gradeObj) => gradeObj.grade
      );

      // the grades that were in the old array and not in the new
      const gradesDeleted = oldStudyGroupGrades.filter(
        (grade) => !newGradesArray.includes(grade)
      );
      gradesDeleted.length &&
        (await this.removeStudentsFromStudyGroup(
          body.id,
          gradesDeleted,
          schoolId
        ));
   ...
   *
   */
  async removeStudentsFromStudyGroup(
  ...
  async addStudentsToStudyGroup(studentsIds: number[], studyGroupId: number) {
    ...
    await this.studyGroupRepository.manager.transaction(
  ...
  async deleteAdminStudyGroups(
    body: AdminTableDto<StudyGroupsRow>,
    schoolId: number
  ) {
    ...
    if (body.params.allChecked) {
      ...
      if (userSearch) {
        deleteStudyGroupsQuery = deleteStudyGroupsQuery.andWhere(
          "id IN (:...deleteByUserSearch)",
          {
            deleteByUserSearch,
          }
        );
